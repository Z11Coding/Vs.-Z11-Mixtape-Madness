...
function bullSpawn(type, xx, yy, hsp, vsp, dmg, ang, exArg, exArg2)
	...
	makeLuaSprite(name, 'mechanics/ut/bullets/'..spr, boxX + xx, boxY + yy);
	...
	if (tp == 'blaster') then
		...
		elseif (s == 1) then
			...
			if (t == 48) then
				...
				makeLuaSprite(n, 'mechanics/ut/bullets/blasterOpen', boxX + bProp[n..'.x'], boxY + bProp[n..'.y']);
...
function onCreate()
	...
	makeLuaSprite('u', 'mechanics/ut/uboa', boxX, boxY);
...
function onUpdate(elapsed)
	...
	if (getProperty('health') <= 0 and not difficultyName == 'Mega Unreal') then
		killAll();
	end
