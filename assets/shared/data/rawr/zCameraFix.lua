...
function updateCam(t, dt)
	...
	if (t == cams.game and type(x) == "number") then
		setProperty("camFollow.x", x + t.scrollOffset.x)
		setProperty("camFollow.y", getProperty("camFollow.y") + t.scrollOffset.x)
	else
...
function updateCamEarly(t, dt)
	if (t == cams.game) then
		setProperty("camFollow.x", getProperty("camFollow.x") - t._lastScrollOffset.x)
		setProperty("camFollow.y", getProperty("camFollow.y") - t._lastScrollOffset.y)
	else
