...
function onCreatePost()
    ...
    makeShader('pixel', {'game', 'hud'})
    makeShader('vcr', {'game', 'hud'})
    makeShader('BarrelBlurEffect', {'game', 'hud'})
    makeShader('MirrorRepeatEffect', {'game', 'hud'})
    makeShader('BloomEffect', {'game', 'hud'})
    makeShader('BlurEffect', {'game', 'hud'})
    makeShader('ChromAbEffect', {'game', 'hud'})
    makeShader('GreyscaleEffect', {'game', 'hud'})
    makeShader('MosaicEffect', {'game', 'hud'})
    makeShader('ScanlineEffect', {'game', 'hud'})
    makeShader('VignetteEffect', {'game', 'hud'})

    makeControl("bilinear_aa", "iTime", 0);
    
    makeControl("vcr", "rOffset", 0);
	makeControl("vcr", "gOffset", 0);
	makeControl("vcr", "bOffset", 0);
    
    makeControl('pixel', 'pxSize', 0.1)

    makeControl('BarrelBlurEffect','barrel',0.0)
    makeControl('BarrelBlurEffect','effect',0.0)
    makeControl('BarrelBlurEffect','zoom',1.0)
    makeControl('BarrelBlurEffect', 'doChroma', false)

    makeControl('BloomEffect','strength',0.0)
    makeControl('BloomEffect','effect',0.0)
    makeControl('BloomEffect','contrast',0.0)
    makeControl('BloomEffect','brightness',0.0)

    makeControl('ChromAbEffect','strength',0)

    makeControl('GreyscaleEffect','strength',0)

    makeControl('MirrorRepeatEffect','x',0.0)
    makeControl('MirrorRepeatEffect','y',0.0)
    makeControl('MirrorRepeatEffect','angle',0.0)
    makeControl('MirrorRepeatEffect','zoom', 1.0)

    makeControl('MosaicEffect','strength',0)

    makeControl('ScanlineEffect','strength',0.0)
    makeControl('ScanlineEffect','pixelsBetweenEachLine',10)
    makeControl('ScanlineEffect','lineHeight',0.6)

    makeControl('VignetteEffect','size',0)
    makeControl('VignetteEffect','stretch',0)
    makeControl('VignetteEffect','red',0)
    makeControl('VignetteEffect','green',0)
    makeControl('VignetteEffect','blue',0)
