--Settings
local Icon = 'sonicring' -- Icon Name (Put It In Images Folder)
local Name = "Vs Sonic.exe" -- Window Name
...
local Info = true -- More Info For Window Name  
local Difficulty = true -- Shows Difficulty For Info

--Watermark Settings   
local Watermark = true -- Puts Text At The Bottom
local Psych_Version = true -- Tells You What Version Of Psych Engine Your Using.
local Watermark_FNF = true -- Puts (FNF:) For Watermark
local Watermark_Difficulty = true -- Shows Difficulty For Watermark
local Watermark_Font = false -- Custom Font For Watermark | Name The Font (Watermark) And Put It In The Fonts Folder
local FontType = '.ttf' -- (.ttf) Or (.otf) For Watermark Font
...
if Watermark_Font == false then
local Font = 'Default Font.ttf'
setTextFont('songText', Font)
end

if Psych_Version == true then
local Font = 'Default Font.ttf'
setTextFont('Psych', Font)
end
...
if (not downscroll) and Watermark == true and Psych_Version == true then
  makeLuaText('Psych', " ", 0, 2, 687);
    setTextAlignment('Psych', 'left');
    setTextSize('Psych', 15);
    setTextBorder('Psych', 1, '000000');
    addLuaText('Psych');
end

if (downscroll) and Watermark == true and Psych_Version == true then
  makeLuaText('Psych', " ", 0, 2, 18);
    setTextAlignment('Psych', 'left');
    setTextSize('Psych', 15);
    setTextBorder('Psych', 1, '000000');
    addLuaText('Psych');
...
if Psych_Version == true then
    setTextString('Psych', "Psych Engine "..version.."");
end
end
