...
function opponentNoteHit()
    ...
    if currentBarPorcent < 1 then
        currentBarPorcent = currentBarPorcent + 0.0020
    end
end
function goodNoteHit()
    if currentBarPorcent > 0 then
        currentBarPorcent = currentBarPorcent - 0.0015
    end
end
...
function onBeatHit()
    if curStep >= 1384 and curStep < 2143 then
        if getProperty('health') > 0.075 and currentBarPorcent < 1 or currentBarPorcent == 1 then
