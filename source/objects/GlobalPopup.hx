...
class ArchPopup extends openfl.display.Sprite {
	...
	public function new(name:String, desc:String, ?image:String, ?onFinish:Void->Void)
	{
		super();
		...
		if(graphic == null) graphic = Paths.image('emptyAchievement');
    ...
    @:allow(objects.GlobalPopup)
	private static var _popups:Array<GlobalPopup> = [];

	public static var showingPopups(get, never):Bool;
	public static function get_showingPopups()
		return _popups.length > 0;

    public static function startPopup(massage:String, desc:String, image:String, ?endFunc:Void->Void = null) {
		...
		var newPop:GlobalPopup = new GlobalPopup(massage, desc, null, image, endFunc);
		_popups.push(newPop);
