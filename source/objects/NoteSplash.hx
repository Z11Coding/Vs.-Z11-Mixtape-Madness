...
class NoteSplash extends NoteObject
{
	...
	public function setupNoteSplash(x:Float, y:Float, direction:Int = 0, ?note:Note = null) {
		...
		if(PlayState.isPixelStage || !ClientPrefs.data.globalAntialiasing) antialiasing = false;
	...
	function loadAnims(skin:String) {
		...
		while(true) {
			...
			for (i in 0...(PlayState.mania+1)) {
				if (!addAnimAndCheck('note$i-$animID', '$animName ${Note.keysShit.get(PlayState.mania).get('letters')[i]} $animID', 24, false)) //trace('maxAnims: $maxAnims');
				{
					return config;
				}
			}
