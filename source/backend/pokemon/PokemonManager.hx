...
class CharacterSprite {
    ...
    public function new(character:String, type:CharacterType) {
        ...
        switch type {
            case NPC:
                ...
                if (!FileSystem.exists(spritePath)) throw "Overworld sprite not found for NPC: " + character;
    ...
    private function loadSprite(path:String, spriteType:String):String {
        if (!FileSystem.exists(path)) throw spriteType + " sprite not found at path: " + path;
        return path;
    }
    ...
    public function loadFrontSprite():FlxSprite {
        if (front == null) throw "Front sprite not available";
        return new FlxSprite(0, 0, front);
    }

    public function loadBackSprite():FlxSprite {
        if (back == null) throw "Back sprite not available";
...
typedef PokeSpecies = {
    ...
    var baseStats:{attack:Int, defense:Int, speed:Int, specialAttack:Int, specialDefense:Int};
    var moves:PokeMoves;
...
class PokemonManager {
    ...
    public extern inline overload function addPokemonToTrainer(trainer:PokeTrainer, id:Int):Void {
        ...
        if (species == null) throw "Species not found for ID: " + id;
