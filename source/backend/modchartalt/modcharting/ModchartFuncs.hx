...
import backend.modchartalt.modcharting.ModchartUtil;
...
class ModchartFuncs
{
    public static function loadLuaFunctions(#if (PSYCH && PSYCHVERSION >= "0.7") funkin:State #end)
    {
        #if PSYCH
        #if LUA_ALLOWED
        #if !(PSYCHVERSION >= "0.7")
        for (funkin in PlayState.instance.luaArray)
        {
            ...
            Lua_helper.add_callback(funkin, 'setModTargetLane', function(name:String, value:Int){
                setModTargetLane(name, value);
            });
            Lua_helper.add_callback(funkin, 'setModPlayfield', function(name:String, value:Int){
                setModPlayfield(name,value);
            });
            Lua_helper.add_callback(funkin, 'addPlayfield', function(?x:Float = 0, ?y:Float = 0, ?z:Float = 0){
                addPlayfield(x,y,z);
            });
            Lua_helper.add_callback(funkin, 'removePlayfield', function(idx:Int){
                removePlayfield(idx);
            });
            Lua_helper.add_callback(funkin, 'tweenModifier', function(modifier:String, val:Float, time:Float, ease:String, ?tag:String = null){
                tweenModifier(modifier,val,time,ease,null,tag);
            });
            Lua_helper.add_callback(funkin, 'tweenModifierSubValue', function(modifier:String, subValue:String, val:Float, time:Float, ease:String, ?tag:String = null){
                tweenModifierSubValue(modifier,subValue,val,time,ease,null,tag);
            });
            Lua_helper.add_callback(funkin, 'setModEaseFunc', function(name:String, ease:String){
                setModEaseFunc(name,ease);
            });
            Lua_helper.add_callback(funkin, 'set', function(beat:Float, argsAsString:String){
                set(beat, argsAsString);
            });
            Lua_helper.add_callback(funkin, 'ease', function(beat:Float, time:Float, easeStr:String, argsAsString:String, ?tag:String = null){
