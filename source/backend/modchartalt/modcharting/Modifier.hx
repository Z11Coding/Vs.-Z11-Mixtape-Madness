...
import backend.modchartalt.modcharting.ModchartFile.CustomModifierScript;
...
class SwapPlayfieldModifier extends Modifier
{
	override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
	{
		var swap = 640;
...
class ReverseStrumsModifier extends Modifier
{
	var scrollSwitch = 520;
...
class ReverseNotesModifier extends Modifier
{
	var scrollSwitch = 520;
...
class MiddleModifier extends Modifier
{
	var posPlayer = 320;
	var posOP = 640;
	...
	override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
	{
		if (lane >= 2 && lane <= 3)
		{
			noteData.x += posOP * currentValue;
		}
		if (lane >= 4 && lane <= 7)
		{
			noteData.x -= posPlayer * currentValue;
		}
	}
...
class GlitchSpeedModifier extends Modifier
{
	var beat = Modifier.beat;

	override function noteDistMath(noteDist:Float, lane:Int, curPos:Float, pf:Int)
	{
		return noteDist * currentValue * FlxG.random.int(1, 2) * FlxMath.fastSin(0.2);
	}
}

class ExtraBoomModifier extends Modifier
{
	override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
	{
		var scrollSwitch = -1;
		...
		noteData.y += (FlxMath.fastSin((curPos / -400)) * 450 + (curPos / 3.5)) * scrollSwitch * (-currentValue);
		noteData.alpha *= FlxMath.bound(1 - (curPos / -450 / 4), 0, 2);
	}
...
class WaveLaneModifier extends Modifier
{
	override function strumMath(noteData:NotePositionData, lane:Int, pf:Int)
	{
		noteData.angle += 260 * currentValue * FlxMath.fastSin(((Conductor.songPosition) * (1) * 0.0008) + (lane / 4)) * 0.2;
	}
	...
	override function incomingAngleMath(lane:Int, curPos:Float, pf:Int)
	{
		return [
			0,
			260 * currentValue * FlxMath.fastSin(((Conductor.songPosition) * (1) * 0.0008) + (lane / 4)) * 0.2
		];
	}
}
...
class TanDrunkAngleModifier extends Modifier
{
	...
	override function noteMath(noteData:NotePositionData, lane:Int, curPos:Float, pf:Int)
	{
		noteData.angle += currentValue * (Math.tan(((Conductor.songPosition * (0.001 * subValues.get('period').value))
			+ ((lane % NoteMovement.keyCount) * 0.2)
			+ (curPos * (0.225 * subValues.get('offset')
				.value)) * ((subValues.get('spacing')
					.value * 10) / FlxG.height)) * (subValues.get('speed').value * 0.2)) * Note.swagWidth * (0.5 * subValues.get('size').value));
