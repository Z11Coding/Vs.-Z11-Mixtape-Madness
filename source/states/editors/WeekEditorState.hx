...
class WeekEditorState extends MusicBeatState
{
	...
	function addWeekUI() {
		...
		category = new FlxUIInputText(130, weekFileInputText.y + 40, 100, '', 8);
		blockPressWhileTypingOn.push(category);
		reloadWeekThing();
		...
		tab_group.add(new FlxText(category.x, category.y - 18, 0, 'Category:'));
	...
	function reloadAllShit() {
		...
		category.text = weekFile.category;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			} else if(sender == category) {
				weekFile.category = category.text.trim();
			}
		}
	}
