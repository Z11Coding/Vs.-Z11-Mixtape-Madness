...
class OutdatedState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bg.loadGraphic(Paths.image('loading/7'));
		...
		for (i in 0...anims.length) {
			...
			b.frames = Paths.getSparrowAtlas("loading/outdatedAssets");
		...
		var txt:FlxText = new FlxText(0, 10, FlxG.width,
			"HEY ! Your Mixtape Engine is outdated ! (You also failed the vibe check...)\n"
			+ 'v$localVer < v$latestVer\n'
			,32);
		txt.setFormat(Paths.font("FridayNightFunkin.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		txt.screenCenter(X);
		add(txt);

		var changelog = new FlxText(100, txt.y + txt.height + 20, 1080, changelog, 16);
		changelog.setFormat(Paths.font("FridayNightFunkin.ttf"), Std.int(16), FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(changelog);
	}
	...
	override function update(elapsed:Float)
	{
		if (FlxG.keys.justPressed.ENTER)
		{
			...
			#else
				FlxG.openURL('https://github.com/Z11Coding/Z11-s-Modpack-Mixup-RELEASE/releases/latest');
			#end
		}
