...
class HeatwaveShader extends MusicBeatState
{
    ...
    public function new():Void{
        super();
        shader.distortTexture.input = BitmapData.fromFile(Paths.getSharedPath('images/heatwave.png'));
    }
...
class FabsShaderGLSL extends FlxShader
{
    @:glFragmentSource('
        ...
        void main(){
                ...
                dst_offset -= vec2(.5,.5);
                dst_offset *= 2.;
                dst_offset *= 0.019; //THIS CONTROLS THE INTENSITY [higher numbers = MORE WAVY]

                //reduce effect towards Y top
                dst_offset *= pow(p_m.t, 2.8); //THIS CONTROLS HOW HIGH UP THE SCREEN THE EFFECT GOES [higher numbers = less screen space]
