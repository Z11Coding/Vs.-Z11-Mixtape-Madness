...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	public function new(dialogueList:DialogueFile, ?song:String = null)
	{
		...
		var escKeys:Array<FlxKey>;

		escKeys = ClientPrefs.keyBinds.get('back').copy();

		var funnyHelp:FlxText = new FlxText(0, 0, 0, 'PRESS ' + InputFormatter.getKeyName(escKeys[0]) + ' OR ' + InputFormatter.getKeyName(escKeys[1]) + ' TO SKIP THE DIALOGUE', 24);
		funnyHelp.setFormat(Paths.font("FridayNightFunkin.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(funnyHelp);
	...
	function startNextDialog():Void
	{
		...
		if(ClientPrefs.data.username) StringTools.replace(curDialogue.text, 'USERNAME', Sys.environment()["USERNAME"]);
		...
		if(daText.sound == null || daText.sound.trim() == '') daText.sound = 'defA';
